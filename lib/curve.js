...
// Per-half-day time shift used by computeAdaptive when the user configured
// sunrise/sunset offsets. The two offsets are independent because users
// typically want asymmetric behaviour (e.g. brighten 30 min before sunrise
// but stay bright 60 min after sunset). We split the day at solar noon --
// any visible discontinuity at noon is negligible because the elevation
// ramp is at its flat plateau there.
function applyTransitionOffset(now, lat, lon, transitions) {
  if (!transitions || (!transitions.sunriseOffsetMin && !transitions.sunsetOffsetMin)) return now;
  if (lat == null || lon == null) return now;
  try {
    const times = SunCalc.getTimes(now, lat, lon);
    const noon = times.solarNoon;
    if (!noon || isNaN(noon.getTime())) return now;
    const offMin = (now < noon ? transitions.sunriseOffsetMin : transitions.sunsetOffsetMin) || 0;
    // Shift the time we feed to SunCalc *backwards* by the offset: a
    // positive offset means "behave as if it's still N minutes earlier",
    // i.e. lights stay bright longer.
    return new Date(now.getTime() - offMin * 60 * 1000);
  } catch (_) {
    return now;
  }
}

function computeAdaptive({ lat, lon, now, day, night, nightMode, transitions }) {
  if (nightMode) {
    const out = {
      kelvin: night.kelvin,
      dimPct: night.dim,
  ...
  const effectiveNow = applyTransitionOffset(now, lat, lon, transitions);
  const pos = SunCalc.getPosition(effectiveNow, lat, lon);
...
function buildDailyCurve({ homey, day, night, transitions }) {
  let lat, lon;
  try {
  ...
  for (let i = 0; i <= 96; i++) {
    ...
    const v = computeAdaptive({ lat, lon, now: t, day, night, transitions, nightMode: false });
    points.push({
      hour: i / 4,
      kelvin: v.kelvin,
      dimPct: v.dimPct,
